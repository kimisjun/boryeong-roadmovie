...
function setupShare(){document.querySelectorAll('[data-share]').forEach(b=>b.addEventListener('click',async()=>{const data={title:document.title,text:'대천해수욕장 2박 3일 여행',url:location.href};try{if(navigator.share)await navigator.share(data);else{await navigator.clipboard.writeText(location.href);toast('링크 복사 완료')}}catch(e){if(e.name!=='AbortError')toast('주소창 링크를 복사해 주세요')}}))}
document.addEventListener('DOMContentLoaded',()=>{setupPlan();setupDays();setupChecks();setupShare()});})();
